...
export const Tag = ({ title, type }: { title: string; type: TagType }) => {
    const baseClass =
        "inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1";

    const styles: Record<TagType, string> = {
        man: "bg-blue-50 text-blue-700 ring-blue-700/10 dark:bg-blue-400/10 dark:text-blue-400 dark:ring-blue-400/30",
        woman:
    ...
    return <span className={`${baseClass} ${styles[type]}`}>{title}</span>;
};
